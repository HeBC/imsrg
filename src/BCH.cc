...
// ============================================================================
// Ordered factorized BCH transforms and two-body source caches
// Part of imsrg++; distributed under the GNU GPL v2 or later.
// ============================================================================
namespace {

void Validate(const Operator &op, const Operator &omega)
{
  for (const auto *x : {&op, &omega})
  {
    if (!x->modelspace || x->modelspace != op.modelspace)
      throw std::invalid_argument("Factorized BCH requires a shared ModelSpace");
    if (x->GetJRank() != 0 || x->GetTRank() != 0 || x->GetParity() != 0 ||
        !x->IsNumberConserving() || x->GetParticleRank() > 2 || x->ThreeBody.IsAllocated())
      throw std::invalid_argument("Factorized BCH requires scalar, number-conserving operators of particle rank <=2");
  }
  if (!op.IsHermitian() || op.IsAntiHermitian() ||
      !omega.IsAntiHermitian() || omega.IsHermitian())
    throw std::invalid_argument("Factorized BCH requires a Hermitian observable and anti-Hermitian Omegas");
  if (Commutator::use_imsrg3 || use_brueckner_bch || use_goose_tank_correction || bch_skip_ieq1)
    throw std::invalid_argument("Factorized BCH does not support IMSRG(3), Brueckner, goose-tank, or skip-i=1 modes");
  if (!(bch_transform_threshold > 0) || !std::isfinite(bch_transform_threshold))
    throw std::invalid_argument("Factorized BCH requires a finite positive transform threshold");
}

Operator Canonical(const Operator &op)
{
  Operator result = op;
  if (result.GetParticleRank() < 2) result.SetParticleRank(2);
  if (result.IsReduced()) result.MakeNotReduced();
  return result;
}

void ReturnFromThreeBody(const Operator &outer, const Operator &inner,
                         const Operator &source, Operator &result)
{
  if (!use_factorized_correction) return;
  // A rank-one input has an identically zero two-body sector, hence no 223
  // source/return. This also accepts one-body-only rotations in the public API.
  if (outer.GetParticleRank() < 2 || inner.GetParticleRank() < 2 || source.GetParticleRank() < 2)
    return;
  if (TurnOn_comm223_231)
    Commutator::FactorizedDoubleCommutator::comm223_231(outer, inner, source, result);
  if (TurnOn_comm223_232)
    Commutator::FactorizedDoubleCommutator::comm223_232(outer, inner, source, result);
}

// Each earlier stage contributes Q_omega(source). Keep the generator paired
// with its weighted two-body source; the stage only borrows these operators.
struct IncomingSource
{
  const Operator &omega;
  const Operator &two_body;
};

// This is the existing immediate-three-body-return approximation extended to
// a nonzero initial high-rank component. It omits propagation 3->3, just as
                                     ...
                                     bool collect_source)
{
  ...
  for (const auto &source : incoming)
  {
    ...
    Validate(source.two_body, omega);
  }
  Operator a = Canonical(omega);
  Operator term = Canonical(input);
  Operator result = term;
  Operator previous = term;
  previous.Erase();
  Operator sum_source;
  if (collect_source) { sum_source = previous; sum_source.TwoBody = term.TwoBody; }
  if (a.Norm() == 0.0)
  {
    if (input.IsReduced())
    {
      result.MakeReduced();
      if (collect_source) sum_source.MakeReduced();
    }
    return {std::move(result), std::move(sum_source)};
  }

  // Scaled terms T_n=N_n/n! avoid the growth of unscaled nested commutators.
  // T_n = [a,T_{n-1}]_<3/n + D_aa(T_{n-2})/[n(n-1)].
  // At n=1, the initial high-rank source contributes D_ab(S_b), coefficient 1.
  bool converged = false;
  Operator correction = previous;
  const bool has_two_body_omega = a.TwoBodyNorm() != 0.0;
  for (int n = 1; n <= 40; ++n)
  {
    Operator next = Commutator::Commutator(a, term);
    next /= double(n);
    correction.Erase();
    ...
    else if (n >= 2 && has_two_body_omega)
    {
      ReturnFromThreeBody(a, a, previous, correction);
      correction /= double(n * (n - 1));
    }
    next += correction;
    result += next;
    if (collect_source) sum_source.TwoBody += (1.0 / (n + 1)) * next.TwoBody;
    const double norm = next.Norm();
    if (!std::isfinite(norm) || !std::isfinite(next.ZeroBody))
      throw std::runtime_error("Factorized BCH generated a non-finite nested term");
    // Two consecutive small terms are needed for this second-order recurrence.
    converged = n >= 2 && norm < bch_transform_threshold && term.Norm() < bch_transform_threshold;
    previous = std::move(term);
    term = std::move(next);
    if (converged) break;
  }
  if (!converged)
    throw std::runtime_error("Factorized BCH did not converge within 40 nested commutators");
  if (input.IsReduced())
  {
    result.MakeReduced();
    if (collect_source) sum_source.MakeReduced();
...
std::pair<Operator, Operator> BCH_TransformWithSource(const Operator &op, const Operator &omega)
{
  if (!use_factorized_correction)
  {
    // No retained high-rank source in the ordinary transform. Preserve the
    // input's quantum numbers and representation, including tensor/IMSRG(3).
    Operator source = op;
    source.Erase();
    return {BCH_Transform(op, omega), std::move(source)};
  }
  return Transform(op, omega, {}, true);
}

Operator BCH_TransformFromSource(const Operator &transformed, const Operator &outer,
                                const Operator &inner, const Operator &source)
{
  if (!use_factorized_correction) return BCH_Transform(transformed, outer);
  return Transform(transformed, outer, {{inner, source}}, false).first;
}

Operator BCH_Transform(const Operator &op, const Operator &outer, const Operator &inner)
{
  if (!use_factorized_correction)
    return BCH_Transform(BCH_Transform(op, inner), outer);
  Validate(op, outer);
  Validate(op, inner);
  auto prepared = BCH_TransformWithSource(op, inner);
  return BCH_TransformFromSource(prepared.first, outer, inner, prepared.second);
}

std::pair<Operator, Operator> BCH_TransformWithSource(const Operator &op, const Operator &omega,
                                                   const Operator &inner, const Operator &source)
{
  if (!use_factorized_correction) return BCH_TransformWithSource(op, omega);
  return Transform(op, omega, {{inner, source}}, true);
}

Operator BCH_TransformFromSources(const Operator &op, const Operator &outer,
                                 const Operator &inner1, const Operator &source1,
                                 const Operator &inner2, const Operator &source2)
{
  if (!use_factorized_correction) return BCH_Transform(op, outer);
  return Transform(op, outer, {{inner1, source1}, {inner2, source2}}, false).first;
}

Operator BCH_Transform(const Operator &op, const Operator &outer,
                       const Operator &middle, const Operator &inner)
{
  if (!use_factorized_correction)
    return BCH_Transform(BCH_Transform(BCH_Transform(op, inner), middle), outer);
  Validate(op, outer);
  Validate(op, middle);
  Validate(op, inner);
  auto first = BCH_TransformWithSource(op, inner);
  auto second = BCH_TransformWithSource(first.first, middle, inner, first.second);
  return BCH_TransformFromSources(second.first, outer, inner, first.second, middle, second.second);
}
}// namespace BCH
