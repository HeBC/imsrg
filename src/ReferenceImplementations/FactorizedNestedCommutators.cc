...
#pragma omp parallel for schedule(dynamic, 1)
  ...
  */
  ...
  namespace
  {
    void ValidateDistinctOmega(const Operator &EtaOuter, const Operator &EtaInner,
                               const Operator &Gamma, const Operator &Z)
    {
      for (const Operator *op : {&EtaOuter, &EtaInner, &Gamma, &Z})
      {
        ...
        if (op->GetJRank() != 0 || op->GetTRank() != 0 || op->GetParity() != 0)
          throw std::invalid_argument("Distinct-Omega reference currently requires parity-even, charge-conserving scalar operators");
      ...
      if (!EtaOuter.IsAntiHermitian() || EtaOuter.IsHermitian()
          || !EtaInner.IsAntiHermitian() || EtaInner.IsHermitian())
      ...
      if (Z.IsHermitian() != Gamma.IsHermitian()
          || Z.IsAntiHermitian() != Gamma.IsAntiHermitian())
        throw std::invalid_argument("Z must have the same Hermiticity as Gamma");
    }

    Operator ScalarTwoBodyPart(const Operator &X)
    {
      // Copy only the 2b sector, preserving its representation. No higher-rank
      // input data are copied or used by the direct contractions.
      Operator X2(*X.modelspace, 0, 0, 0, 2);
      if (X.IsAntiHermitian()) X2.SetAntiHermitian();
      if (X.IsReduced()) X2.MakeReduced();
      X2.TwoBody = X.TwoBody;
      if (X2.IsReduced()) X2.MakeNotReduced();
      return X2;
    }

    // Direct substitution of the 223 two-body products into the 231 contraction.
    // Contract223(Gamma,EtaInner) evaluates each requested scalar contraction
    // directly; no three-body operator, array, or matrix-element cache is formed.
    void Direct223231(const Operator &EtaOuter, const Operator &EtaInner,
                      const Operator &Gamma, Operator &Z)
    {
      auto &ms = *Z.modelspace;
      const auto &A2 = EtaOuter.TwoBody;
      const auto &orbits = ms.all_orbits;
      for (size_t i : orbits)
      {
        const Orbit &oi = ms.GetOrbit(i);
        for (size_t j : Z.GetOneBodyChannel(oi.l, oi.j2, oi.tz2))
        {
          if (j < i) continue; // The result is Hermitian.
            ...
            for (size_t b : orbits)
            {
              ...
              for (size_t c : orbits)
              {
                ...
                for (size_t d : orbits)
                {
                  ...
                  const double occupation = oa.occ * ob.occ * (1-oc.occ) * (1-od.occ);
                  ...
                  for (int Jab = Jmin; Jab <= Jmax; ++Jab)
                  {
                    if ((a == b || c == d) && Jab%2 != 0) continue;
                    const double aabcd = A2.GetTBME_J(Jab,Jab,a,b,c,d);
                    const double acdab = A2.GetTBME_J(Jab,Jab,c,d,a,b);
                    if (aabcd == 0.0 && acdab == 0.0) continue;
                    const int twoJmin = std::max(std::abs(2*Jab-oi.j2),std::abs(2*Jab-oj.j2));
                    const int twoJmax = std::min(2*Jab+oi.j2,2*Jab+oj.j2);
                    for (int twoJ = twoJmin; twoJ <= twoJmax; twoJ += 2)
                    {
                      const double scdiabj = Commutator::FactorizedDoubleCommutator::detail::Contract223(Gamma,EtaInner,Jab,Jab,twoJ,c,d,i,a,b,j);
                      const double sabicdj = Commutator::FactorizedDoubleCommutator::detail::Contract223(Gamma,EtaInner,Jab,Jab,twoJ,a,b,i,c,d,j);
                      // Minus the source 231 expression, since the outer order is [S,A].
                      zij -= 0.25 * occupation * (twoJ+1.0)/(oi.j2+1.0)
                             * (aabcd*scdiabj - sabicdj*acdab);
                    }
                  }
                }
              }
    ...
    void Direct223232(const Operator &EtaOuter, const Operator &EtaInner,
                      const Operator &Gamma, Operator &Z)
    {
      const auto &A2 = EtaOuter.TwoBody;
      auto &Z2 = Z.TwoBody;
  ...
  #pragma omp parallel for schedule(dynamic, 1)
      for (size_t ich = 0; ich < nchans; ich++)
      {
        ...
        for (int ibra = 0; ibra < nbras; ibra++)
        {
          ...
          for (int iket = ket_min; iket < nkets; iket++)
          {
            ...
            for (auto c : Z.modelspace->all_orbits)
            {
              ...
              for (int ch_ab = 0; ch_ab < nch; ch_ab++)
              {
                ...
                for (size_t iket_ab = 0; iket_ab < nkets_ab; iket_ab++)
                {
                  ...
                  bool xicab_good = ((oi.l + oc.l + tbc_ab.parity) % 2 == 0) and (std::abs(oi.tz2 + oc.tz2 - 2 * tbc_ab.Tz) == 2 * 0);
                  bool xjcab_good = ((oj.l + oc.l + tbc_ab.parity) % 2 == 0) and (std::abs(oj.tz2 + oc.tz2 - 2 * tbc_ab.Tz) == 2 * 0);
                  bool xabck_good = ((ok.l + oc.l + tbc_ab.parity) % 2 == 0) and (std::abs(ok.tz2 + oc.tz2 - 2 * tbc_ab.Tz) == 2 * 0);
                  bool xabcl_good = ((ol.l + oc.l + tbc_ab.parity) % 2 == 0) and (std::abs(ol.tz2 + oc.tz2 - 2 * tbc_ab.Tz) == 2 * 0);

                  // Xicab term
                  if (xicab_good and (std::abs(oi.j2 - oc.j2) <= 2 * Jab) and (oi.j2 + oc.j2 >= 2 * Jab))
                  {

                    int twoJ_min = std::max(std::abs(oc.j2 - 2 * J), std::abs(oj.j2 - 2 * Jab));
                    int twoJ_max = std::min(oc.j2 + 2 * J, oj.j2 + 2 * Jab);
                    double xciab = A2.GetTBME_J(Jab, c, i, a, b);
                    int phasefactor = Z.modelspace->phase((oi.j2 + oj.j2) / 2 - J);
                    if (xciab != 0.0)
                    for (int twoJ = twoJ_min; twoJ <= twoJ_max; twoJ += 2)
                    {

                      double sixj = AngMom::SixJ(jj, ji, J, jc, 0.5*twoJ, Jab);
                      double hatfactor = (twoJ + 1) * sqrt((2 * Jab + 1.) / (2 * J + 1));
                      const double sabjklc = Commutator::FactorizedDoubleCommutator::detail::Contract223(Gamma, EtaInner, Jab, J, twoJ, a, b, j, k, l, c);
                      zijkl += occfactor * hatfactor * phasefactor * sixj * (xciab * sabjklc);
                    }
                  }

                  // Xjcab term
                  if (xjcab_good and (std::abs(oj.j2 - oc.j2) <= 2 * Jab) and (oj.j2 + oc.j2 >= 2 * Jab))
                  {
                    int twoJ_min = std::max(std::abs(oc.j2 - 2 * J), std::abs(oi.j2 - 2 * Jab));
                    int twoJ_max = std::min(oc.j2 + 2 * J, oi.j2 + 2 * Jab);
                    double xcjab = A2.GetTBME_J(Jab, c, j, a, b);
                    int phasefactor = 1;
                    if (xcjab != 0.0)
                    for (int twoJ = twoJ_min; twoJ <= twoJ_max; twoJ += 2)
                    {
                      double sixj = AngMom::SixJ(ji, jj, J, jc, 0.5*twoJ, Jab);
                      double hatfactor = (twoJ + 1) * sqrt((2 * Jab + 1.) / (2 * J + 1));
                      const double sabiklc = Commutator::FactorizedDoubleCommutator::detail::Contract223(Gamma, EtaInner, Jab, J, twoJ, a, b, i, k, l, c);
                      zijkl -= occfactor * hatfactor * phasefactor * sixj * (xcjab * sabiklc);
                    }
                  }
                  ...
                  if (xabck_good and (std::abs(ok.j2 - oc.j2) <= 2 * Jab) and (ok.j2 + oc.j2 >= 2 * Jab))
                  {
                    ...
                    double xabck = A2.GetTBME_J(Jab, a, b, c, k);
                    int phasefactor = Z.modelspace->phase((ok.j2 + ol.j2) / 2 - J);
                    if (xabck != 0.0)
                    for (int twoJ = twoJ_min; twoJ <= twoJ_max; twoJ += 2)
                    {
                      double sixj = AngMom::SixJ(jl, jk, J, jc, 0.5*twoJ, Jab);
                      double hatfactor = (twoJ + 1) * sqrt((2 * Jab + 1.) / (2 * J + 1));
                      const double sijcabl = Commutator::FactorizedDoubleCommutator::detail::Contract223(Gamma, EtaInner, J, Jab, twoJ, i, j, c, a, b, l);
                      zijkl -= occfactor * hatfactor * phasefactor * sixj * (sijcabl * xabck);
                    }
                  }
                  ...
                  if (xabcl_good and (std::abs(ol.j2 - oc.j2) <= 2 * Jab) and (ol.j2 + oc.j2 >= 2 * Jab))
                  {
                    ...
                    double xabcl = A2.GetTBME_J(Jab, a, b, c, l);
                    int phasefactor = 1;
                    if (xabcl != 0.0)
                    for (int twoJ = twoJ_min; twoJ <= twoJ_max; twoJ += 2)
                    {
                      double sixj = AngMom::SixJ(jk, jl, J, jc, 0.5*twoJ, Jab);
                      double hatfactor = (twoJ + 1) * sqrt((2 * Jab + 1.) / (2 * J + 1));
                      const double sijcabk = Commutator::FactorizedDoubleCommutator::detail::Contract223(Gamma, EtaInner, J, Jab, twoJ, i, j, c, a, b, k);
                      zijkl += occfactor * hatfactor * phasefactor * sixj * (sijcabk * xabcl);
                    }
                  }
            ...
            // Reverse the outer 23 commutator: [S_3,EtaOuter_2], S=[Gamma,EtaInner]_3.
            // Each S above is expanded immediately into two-body products.
    ...
    void DistinctOmegaReference(const Operator &EtaOuter, const Operator &EtaInner,
                                const Operator &Gamma, Operator &Z, int output_rank)
    {
      ValidateDistinctOmega(EtaOuter, EtaInner, Gamma, Z);
      const double t_start = omp_get_wtime();
      const Operator outer2 = ScalarTwoBodyPart(EtaOuter);
      const Operator inner2 = ScalarTwoBodyPart(EtaInner);
      const Operator gamma2 = ScalarTwoBodyPart(Gamma);

      // The direct kernels use only products of the three two-body inputs.
      // There is no rank-3 allocation and no dependence on three-body cutoffs.
      Z.modelspace->PreCalculateSixJ();
      Operator delta(*Z.modelspace, 0, 0, 0, 2);
      if (output_rank == 1)
        Direct223231(outer2, inner2, gamma2, delta);
      else
        Direct223232(outer2, inner2, gamma2, delta);

      // Convert only the increment, preserving Z's other sectors and allowing
      // Z to alias an input without changing any data used in the calculation.
      if (Z.IsReduced()) delta.MakeReduced();
      if (output_rank == 1)
        Z.OneBody += delta.OneBody;
      else
        Z.TwoBody += delta.TwoBody;
      Z.profiler.timer[output_rank == 1
          ? "ReferenceImplementations::comm223_231_BruteForce_distinct"
          : "ReferenceImplementations::comm223_232_BruteForce_distinct"] += omp_get_wtime() - t_start;
    }
  } // anonymous namespace

  void comm223_231_BruteForce(const Operator &EtaOuter, const Operator &EtaInner,
                            const Operator &Gamma, Operator &Z)
  {
    DistinctOmegaReference(EtaOuter, EtaInner, Gamma, Z, 1);
  }

  void comm223_232_BruteForce(const Operator &EtaOuter, const Operator &EtaInner,
                            const Operator &Gamma, Operator &Z)
  {
    DistinctOmegaReference(EtaOuter, EtaInner, Gamma, Z, 2);
  }
