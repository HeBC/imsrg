...
#pragma omp parallel for schedule(dynamic,1)
      for (int ch=0;ch<nch;++ch)
      {
        auto &tc=Z.modelspace->GetTwoBodyChannel(ch);
        const auto &e=Eta.TwoBody.GetMatrix(ch,ch);
        arma::mat weighted=e;
...
#pragma omp parallel for schedule(dynamic, 1)
      for (int ich=0; ich<nbra_ket_ch; ich++)
      {
        ...
        // (Eta W)^T = -W Eta, with W the occupation-weighted Eta.
...
#pragma omp parallel for schedule(dynamic, 1)
      ...
      for (int ich=0; ich<nch_braket; ich++)
      {
         ...
         for (size_t ibra_cc = 0; ibra_cc < nBras_cc; ++ibra_cc)
         {
           ...
           // loop over cross-coupled kets |cd> in this channel
           for (int iket_cc = (ch_bra_cc == ch_ket_cc ? ibra_cc : 0); iket_cc < nKets_cc * 2; ++iket_cc)
           {
...
#pragma omp parallel for schedule(dynamic, 1)
    ...
    void comm223_232_chi1b(const Operator &Eta, const Operator &Gamma, Operator &Z)
    {
      ...
      {
        Orbit &op = Z.modelspace->GetOrbit(p);
        // Pure Eta-Eta and mixed Gamma-Eta carry different charge changes.
        for (auto q : Z.modelspace->all_orbits)
        {
          const Orbit &oq = Z.modelspace->GetOrbit(q);
          if (op.l != oq.l or op.j2 != oq.j2) continue;
          if (op.tz2 != oq.tz2 and std::abs(op.tz2-oq.tz2) != 2*Gamma.GetTRank()) continue;
          p_list.push_back(p);
          q_list.push_back(q);
        }
      }
            ...
            for (int J_std = jmin; J_std <= jmax; J_std++)
            {
              ...
              if (std::abs(sixj1) > 1e-8)
              {
                ...
                if (indx_ad >= 0 and indx_bc >= 0)
                {
                  ...
                  // The mixed Gamma-Eta transpose carries hEta*hZ, including Hermitian Eta.
