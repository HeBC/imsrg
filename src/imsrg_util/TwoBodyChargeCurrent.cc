...
Operator BuildTwoBodyChargeOperator(
    ModelSpace& modelspace, double momentum_transfer_mev,
    double ge_isoscalar,
    const TwoBodyChargeOperatorParameters& parameters,
    int work_rank, int work_size)
{
  parameters.Validate();
  ...
  if (work_size < 1 || work_rank < 0 || work_rank >= work_size)
  {
    throw std::invalid_argument("Invalid distributed-work rank or size");
  }
  ...
  // Keep complete channel matrices on one rank so the expensive relative-HO
  // and angular-projection caches are reused. Greedy assignment by the
  // approximate magnetic-projection work balances the unequal channel sizes
  // deterministically on every rank without MPI communication.
  std::vector<std::pair<int, std::size_t>> channel_work;
  for (const auto& matrix_entry : result.TwoBody.MatEl)
  {
    ...
    const std::size_t number_kets =
        static_cast<std::size_t>(channel.GetNumberKets());
    const std::size_t number_tbmes = number_kets * (number_kets + 1) / 2;
    const std::size_t cost = number_tbmes
        * static_cast<std::size_t>(2 * channel.J + 1);
    channel_work.emplace_back(chbra, cost);
  }
  std::sort(
      channel_work.begin(), channel_work.end(),
      [](const std::pair<int, std::size_t>& lhs,
         const std::pair<int, std::size_t>& rhs) {
        return lhs.second != rhs.second
            ? lhs.second > rhs.second : lhs.first < rhs.first;
      });
  std::vector<std::size_t> rank_loads(
      static_cast<std::size_t>(work_size), 0);
  std::map<int, int> channel_owner;
  for (const auto& work : channel_work)
  {
    const auto lightest = std::min_element(
        rank_loads.begin(), rank_loads.end());
    const int owner = static_cast<int>(
        std::distance(rank_loads.begin(), lightest));
    channel_owner[work.first] = owner;
    *lightest += work.second;
  }
  ...
  for (auto& matrix_entry : result.TwoBody.MatEl)
  {
    ...
    if (channel_owner.at(chbra) != work_rank)
    {
      continue;
    }
    TwoBodyChannel& channel = modelspace.GetTwoBodyChannel(chbra);
...
bool mpi_initialized_by_two_body_charge = false;

void CheckMPI(int error_code, const char* operation)
{
  ...
  message << operation << " failed: "
          << std::string(error_text, static_cast<std::size_t>(error_length));
  throw std::runtime_error(message.str());
}
...
void RequireMPIInitialized()
{
  ...
  {
    throw std::runtime_error(
        "Two-body charge MPI is not initialized or was already finalized");
  }
}

void AllReduceTwoBodyMatrices(Operator& result)
{
  for (auto& matrix_entry : result.TwoBody.MatEl)
  {
    arma::mat& matrix = matrix_entry.second;
    std::size_t offset = 0;
    while (offset < matrix.n_elem)
    {
      const std::size_t remaining = matrix.n_elem - offset;
      const int count = static_cast<int>(std::min(
          remaining, static_cast<std::size_t>(INT_MAX)));
      CheckMPI(
          MPI_Allreduce(
              MPI_IN_PLACE, matrix.memptr() + offset, count,
              MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD),
          "MPI_Allreduce(two-body charge TBMEs)");
...
Operator TwoBodyChargeOperator(
    ...
    const TwoBodyChargeOperatorParameters& parameters)
{
  return BuildTwoBodyChargeOperator(
      modelspace, momentum_transfer_mev, ge_isoscalar, parameters, 0, 1);
}

bool TwoBodyChargeMPIAvailable()
{
#ifdef IMSRG_ENABLE_MPI
  return true;
#else
...
void TwoBodyChargeMPIInitialize()
{
#ifdef IMSRG_ENABLE_MPI
  int initialized = 0;
  CheckMPI(MPI_Initialized(&initialized), "MPI_Initialized");
  if (!initialized)
  {
    ...
    CheckMPI(
        ...
        "MPI_Init_thread");
    mpi_initialized_by_two_body_charge = true;
    if (provided < MPI_THREAD_FUNNELED)
    {
      MPI_Finalize();
      mpi_initialized_by_two_body_charge = false;
      throw std::runtime_error(
  ...
  {
    int provided = MPI_THREAD_SINGLE;
    CheckMPI(MPI_Query_thread(&provided), "MPI_Query_thread");
    if (provided < MPI_THREAD_FUNNELED)
    {
      throw std::runtime_error(
          "Existing MPI session does not provide MPI_THREAD_FUNNELED");
    }
  }
#else
  throw std::runtime_error(
      "MPI support is unavailable; configure with -DIMSRG_ENABLE_MPI=ON");
#endif
}

void TwoBodyChargeMPIFinalize()
{
#ifdef IMSRG_ENABLE_MPI
  int finalized = 0;
  CheckMPI(MPI_Finalized(&finalized), "MPI_Finalized");
  if (mpi_initialized_by_two_body_charge && !finalized)
  {
...
bool TwoBodyChargeMPIAllTrue(bool local_value)
{
#ifdef IMSRG_ENABLE_MPI
  RequireMPIInitialized();
  const int local_integer = local_value ? 1 : 0;
  int global_integer = 0;
  CheckMPI(
      MPI_Allreduce(
          &local_integer, &global_integer, 1,
          MPI_INT, MPI_MIN, MPI_COMM_WORLD),
      "MPI_Allreduce(all ranks true)");
...
Operator TwoBodyChargeOperatorMPI(
    ModelSpace& modelspace, double momentum_transfer_mev,
    double ge_isoscalar,
    const TwoBodyChargeOperatorParameters& parameters)
{
#ifdef IMSRG_ENABLE_MPI
  RequireMPIInitialized();
  const int rank = TwoBodyChargeMPIRank();
  const int size = TwoBodyChargeMPISize();

  std::unique_ptr<Operator> result;
  std::string local_error;
  try
  {
    result.reset(new Operator(BuildTwoBodyChargeOperator(
        modelspace, momentum_transfer_mev, ge_isoscalar,
        parameters, rank, size)));
  }
  ...
  CheckMPI(
      MPI_Allreduce(
          &local_failure, &global_failure, 1,
          MPI_INT, MPI_MAX, MPI_COMM_WORLD),
      "MPI_Allreduce(two-body charge status)");
  if (global_failure)
  {
    std::ostringstream message;
    message << "Distributed two-body charge construction failed";
    if (local_failure)
    {
      message << " on rank " << rank << ": " << local_error;
    }
    else
    {
      message << " on another MPI rank";
    }
    throw std::runtime_error(message.str());
  }

  AllReduceTwoBodyMatrices(*result);
  return std::move(*result);
#else
  (void)modelspace;
  (void)momentum_transfer_mev;
  (void)ge_isoscalar;
  (void)parameters;
  throw std::runtime_error(
      "MPI support is unavailable; configure with -DIMSRG_ENABLE_MPI=ON");
