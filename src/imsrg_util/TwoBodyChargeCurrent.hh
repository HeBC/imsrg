...
/// Return true when the library was configured with -DIMSRG_ENABLE_MPI=ON.
bool TwoBodyChargeMPIAvailable();

/// Initialize MPI with MPI_THREAD_FUNNELED when it has not already been
/// initialized. The matching finalize call only finalizes MPI when this
/// library performed the initialization.
void TwoBodyChargeMPIInitialize();
void TwoBodyChargeMPIFinalize();

/// MPI world information. Before initialization (or in a non-MPI build),
/// these return rank zero and size one.
...
/// Collective MPI version of TwoBodyChargeOperator(). Every rank in
/// MPI_COMM_WORLD must call this function with the same model space and
/// parameters in the same order. Complete diagonal two-body channel matrices
/// are cost-balanced across ranks and then combined on every rank.
